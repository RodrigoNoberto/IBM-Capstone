...
def get_scatter_chart(site_dropdown, payload_slider):
    ...
    if site_dropdown == 'ALL':
        scatter = px.scatter(df1, x='Payload Mass (kg)', y='class', title='Correlation between Payload and Success for all Sites', color='Booster Version Category')
        return scatter

    else:
        scatter = px.scatter(filtered_py, x='Payload Mass (kg)', y='class', title=f'Correlation between Payload and Success for Site {site_dropdown}', color='Booster Version Category')
        return scatter
